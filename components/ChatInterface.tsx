  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (currentImage) {
        ...
        // ✅ KODE BARU - CEK FUNCTION CALL DENGAN BENAR
        const candidate = response.candidates?.[0];
        let hasFunctionCall = false;
        
        if (candidate?.content?.parts) {
          for (const part of candidate.content.parts) {
            if (part.functionCall && part.functionCall.name === 'generate_visual') {
              hasFunctionCall = true;
              
              if (!checkImageLimit()) {
                aiResponseText = "Sistem: Batas harian visualisasi tercapai. Mohon langganan Rival Premium untuk akses tak terbatas.";
              } else {
                const promptForImage = part.functionCall.args?.prompt || currentInput;
        ...
        // Kalau ga ada function call, ambil text response
        if (!hasFunctionCall) {
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar pt-2 pb-60">
        {session.messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-1000">
             ...
             <p className="text-[11px] opacity-40 uppercase tracking-[0.6em] font-black">RIVAL {activeMode.to} ENGINE</p>
          </div>
        ) : (
