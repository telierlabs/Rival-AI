            ...
            {selectedAsset.imageUrl ? (
              ...
              <div className="w-full h-full rounded-[2.5rem] overflow-hidden border border-white/10 bg-black/40 shadow-2xl">
                {previewMode === 'preview' ? (
                  <iframe 
                    title="Code Preview"
                    srcDoc={prepareIframeContent(selectedAsset.codeSnippet)} 
                    className="w-full h-full bg-white border-none"
                    sandbox="allow-scripts"
                  />
                ) : (
                  <pre className="p-10 text-zinc-300 font-mono text-sm leading-relaxed overflow-auto h-full no-scrollbar">
...
const FilterBtn: React.FC<{ active: boolean; onClick: () => void; label: string }> = ({ active, onClick, label }) => (
  <button 
    onClick={onClick}
    className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${active ? 'bg-current text-white invert' : 'opacity-40 hover:opacity-100'}`}
  >
    {label}
  </button>
);
