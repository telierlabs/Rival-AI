...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Determine accent and border colors based on theme
    ...
    >
      <div className={`h-14 flex items-center justify-between px-6 border-b ${borderColor}`}>
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 ${isDark ? 'bg-white text-black' : 'bg-black text-white'} rounded-xl flex items-center justify-center`}>
            <span className="text-[12px] font-black">R</span>
          </div>
      ...
      <div className="p-4">
        <button 
          onClick={onNewChat}
          className={`w-full flex items-center justify-center gap-2 ${isDark ? 'bg-zinc-100 hover:bg-white text-black' : 'bg-zinc-900 hover:bg-black text-white'} font-bold py-3 px-4 rounded-2xl transition-all active:scale-95 shadow-lg shadow-black/5`}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
          </svg>
          <span className="text-xs uppercase tracking-widest font-black">New Chat</span>
        </button>
      </div>

      <div className="flex-1 overflow-y-auto no-scrollbar px-3 space-y-6 pb-20">
        ...
        {items.map(item => (
          <div key={item.id} className="group relative">
            <button 
              ...
              className={`w-full text-left px-3.5 py-3 rounded-xl transition-all flex flex-col gap-0.5 ${
                activeId === item.id 
            ...
            >
              ...
              <span className={`text-[9px] font-bold ${activeId === item.id ? (isDark ? 'text-zinc-500' : 'text-zinc-400') : 'text-zinc-500'}`}>
